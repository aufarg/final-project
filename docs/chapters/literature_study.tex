...
Proses dalam sebuah partisi diperbolehkan untuk melakukan multitasking.
...
Sistem dengan partisi yang kuat memungkinkan partisi dengan tingkat kekritisan yang berbeda berjalan pada satu \textit{module} yang sama tanpa mempengaruhi partisi lain secara \textit{ruang} dan waktu.
...
Waktu harus unik dan tidak bergantung pada eksekusi partisi apapun dalam \textit{module}.
...
\section{\textit{Primary-Backup Scheduling}}

Studi untuk membuat sistem \textit{real-time} menjadi \textit{fault-tolerant} telah banyak dilakukan \citep{Campbell1986} \citep{Bertossi2006}. \citet{Campbell1986} menunjukkan bahwa penambahan \textit{deadline} pada setiap \textit{task} akan menghasilkan sistem yang \textit{fault-tolerant}.
...
\citet{Jin2013} berhasil membuktikan bahwa penggunaan \textit{primary-backup scheduling} dapat dilakukan pada sistem yang terpartisi dengan melakukan peluasan model \textit{resource}.
...
Xen ARINC 653 merupakan prototipe sistem ARINC 653 yang dibangun diatas Xen.
...
Setiap partisi akan direpresentasikan dengan menggunakan \textit{domain} pada Xen.
